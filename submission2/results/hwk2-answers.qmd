...
Looking in the data, we are able to see that there are `r unique.count <-n_distinct(full.hcris.data$provider_number)` rows, so there are `r unique.count <-n_distinct(full.hcris.data$provider_number)` total plan types. 
